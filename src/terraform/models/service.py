...
class SquadcastService(TerraformResource):
    ...
    def __init__(self, **data):
        ...
        if "escalation_policy" in data and isinstance(
            data["escalation_policy"], TerraformResource
        ):
            data["escalation_policy_id"] = data[
                "escalation_policy"
    ...
    email_prefix: str = Field(..., description="Email prefix for the service")
    ...
    description: Optional[str] = Field(
        None, description="Detailed description about this service"
    )
    alert_sources: Optional[List[str]] = Field(
        None, description="List of active alert source names"
    )
    dependencies: Optional[Set[str]] = Field(
        None, description="Set of service IDs that this service depends on"
    )
    ...
    tags: Optional[List[ServiceTag]] = Field(
        None, description="List of key-value tags for the service"
    )

    # Read-only fields
    id: Optional[str] = Field(None, description="Service ID (read-only)", readonly=True)
    api_key: Optional[str] = Field(
        None, description="Unique API key of this service (read-only)", readonly=True
    )
    email: Optional[str] = Field(
        None, description="Service email (read-only)", readonly=True
    )
    active_alert_source_endpoints: Optional[Dict[str, str]] = Field(
        None, description="Active alert source endpoints (read-only)", readonly=True
    )
